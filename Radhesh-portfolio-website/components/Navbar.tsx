...
export function Navbar({ isDark, onThemeToggle }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-300/80 bg-white/92 backdrop-blur-xl dark:border-slate-700 dark:bg-slate-950/92">
      <Container className="flex h-16 items-center justify-between">
        <a href="#home" className="group flex items-center gap-3" aria-label={`${profile.name} home`}>
          <img
            src="/profile.jpg"
            alt=""
            className="size-10 rounded-lg object-cover object-top border border-slate-200 dark:border-slate-800"
          />
        ...
        <nav aria-label="Primary navigation" className="hidden items-center gap-1 xl:flex">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-primary dark:text-slate-300 dark:hover:bg-slate-900 dark:hover:text-white"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="grid size-10 place-items-center rounded-lg border border-slate-200 bg-white text-primary transition hover:border-accent hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent dark:border-slate-700 dark:bg-slate-900 dark:text-white xl:hidden"
      ...
      {isOpen ? (
        <nav className="border-t border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 xl:hidden" aria-label="Mobile navigation">
